...
public class StaticInterpreter {
	...
	public static Frame execute(Frame f, ConstantPoolGen cpg, InstructionHandle ih) {
		...
		if (i instanceof ACONST_NULL) {
			...
			cur.pop();
			cur.push(new TopValue(Type.INT));
			...
			cur.push(new ConstantValue(cp.getType(cpg), cp.getValue()));
			...
			if (v instanceof ConstantValue) {
				...
				if (type.equals(Type.FLOAT)) {
					cur.push(new ConstantValue(type, n.floatValue()));
				} else if (type.equals(Type.INT)) {
					cur.push(new ConstantValue(type, n.intValue()));
				} else if (type.equals(Type.DOUBLE)) {
					cur.push(new ConstantValue(type, n.doubleValue()));
				} else if (type.equals(Type.LONG)) {
					cur.push(new ConstantValue(type, n.longValue()));
				} else {
					throw new IllegalArgumentException(type.toString());
				}
			} else {
				cur.push(new TopValue(type));
			}
			...
			if (i instanceof ANEWARRAY || i instanceof CHECKCAST || i instanceof INSTANCEOF || i instanceof NEW) {
				...
				if (o instanceof Number) {
					cur.push(new ConstantValue(type, 0));
				} else {
					cur.push(new TopValue(type));
				}
			} else if (i instanceof LDC2_W) {
				LDC2_W ldc = (LDC2_W) i;
				cur.push(new ConstantValue(type, ldc.getValue(cpg)));
			...
			if(lvi instanceof LoadInstruction) {
				cur.push(cur.getLocal(lvi.getIndex()));
